...
export const PortfolioPage: React.FC<PortfolioPageProps> = ({ lang, onOpenLightbox }) => {
  ...
  const visiblePhotos = selected === 'all' ? PORTFOLIO_ITEMS : photosFor(groups.find(group => group.id === selected)!);
  return (
    <div className="pb-20 space-y-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div aria-live="polite" className="text-slate-600">{visiblePhotos.length} {lang === 'en' ? 'photos' : 'foto'}</div>
        <div data-photo-grid className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {visiblePhotos.map(item => (
                <figure key={item.id} data-photo-id={item.id} className="rounded-2xl overflow-hidden border border-slate-200 bg-white shadow-sm hover:shadow-lg transition-shadow">
                  <button type="button" onClick={() => onOpenLightbox(item, visiblePhotos)} aria-label={`${lang === 'en' ? 'View photo' : 'Lihat foto'}: ${item.title[lang]}`} className="block w-full focus-visible:outline-4 focus-visible:outline-emerald-600">
                  ...
                  <figcaption className="p-5 space-y-3">
                    ...
                    <button type="button" onClick={() => onOpenLightbox(item, visiblePhotos)} className="inline-flex items-center gap-2 min-h-11 text-emerald-800 font-semibold"><Eye className="w-5 h-5" />{lang === 'en' ? 'View full photo' : 'Lihat foto penuh'}</button>
                  </figcaption>
                </figure>
              ))}
        </div>
